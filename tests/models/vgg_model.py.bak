...
Vgg_HParams = namedtuple('HParams',
...
class Vgg(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('conv_1_0'):
            x = self._images
            x = self._conv_ReLU('conv1_0', x, 3, 3, 16, self._stride_arr(1))
            tf.nn.dropout(x, self._dropout)
        with tf.variable_scope('conv_1_1'):
            x = self._conv_ReLU('conv1_1', x, 3, 16, 16, self._stride_arr(1))
            tf.nn.dropout(x, self._dropout)
        with tf.variable_scope('pool_1'):
            x = self._max_pool(x, 'pool_1')

        with tf.variable_scope('conv_2_0'):
            x = self._conv_ReLU('conv2_0', x, 3, 16, 32, self._stride_arr(1))
            tf.nn.dropout(x, self._dropout)
        with tf.variable_scope('conv_2_1'):
            x = self._conv_ReLU('conv2_1', x, 3, 32, 32, self._stride_arr(1))
            tf.nn.dropout(x, self._dropout)
        ...
        with tf.variable_scope('conv_3_0'):
            x = self._conv_ReLU('conv3_0', x, 3, 32, 64, self._stride_arr(1))
            tf.nn.dropout(x, self._dropout)
        with tf.variable_scope('conv_3_1'):
            x = self._conv_ReLU('conv3_1', x, 3, 64, 64, self._stride_arr(1))
            tf.nn.dropout(x, self._dropout)
        with tf.variable_scope('pool_3'):
            x = self._max_pool(x, 'pool_3')

        with tf.variable_scope('fc_1'):
            x = self._fully_connected('fc_1', x, 256)
