...
class Isling(object):
    ...
    def _res(self, x, in_filters, out_filters, stride, name, share_activation=False):
        with tf.variable_scope(name):
            with tf.variable_scope('share_activation'):
                if share_activation:
            ...
            with tf.variable_scope('add'):
                if in_filters != out_filters:
                    ori_x = tf.nn.avg_pool(ori_x, stride, stride, 'VALID')
                    ori_x = tf.pad(
                        ori_x, [[0, 0], [0, 0], [0, 0],
